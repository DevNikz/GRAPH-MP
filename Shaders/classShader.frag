...
vec3 CalcPointLight(PointLight light, vec3 normal, vec3 fragPos, vec3 viewDir, vec3 texColor)
{
    ...
    att *= rangeAtt;
    //att *= clamp(1.0 - (distance - light.range), 0.0, 1.0);
...
void main()
{
    ...
    result += CalcPointLight(pointLight, normal, fragPos, viewDir, blendedRGB);
    result += CalcDirLight(dirLight, normal, viewDir, blendedRGB);
